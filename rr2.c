...
main (void)
{
  ...
  char caSound[9] = { 'N', 'o', 'n', 'e', '\0' };	// sound of the gun when an action is taken
  char caErrMsg[51] = "";	// error message in JSON package
...
// test for a null QUERY_STRING (applicable when submitted from the console) -------------------------------------------
  ...
  if (sQueryString == NULL)
    {
      printf
	("QUERY_STRING is NULL. Expecting QUERY_STRING=\"Action=<action>\". Ending json4Ajax.cgi");
      printf ("\n\n");
      return EXIT_FAILURE;
    }
...
// test for a empty QUERY_STRING (applicable when submitted from the console) ------------------------------------------
  ...
  if (sQueryString[0] == '\0')
    {
      printf
	("QUERY_STRING is empty. Expecting QUERY_STRING=\"Action=<action>\". Ending json4Ajax.cgi");
      printf ("\n\n");
      return EXIT_FAILURE;
    }
...
// test for a valid parameter name in QUERY_STRING ---------------------------------------------------------------------
  ...
  if (sAction[0] == '\0')
    {
      printf
	("QUERY_STRING is \"%s\". Expecting QUERY_STRING=\"Action=<action>\". Ending json4Ajax.cgi",
	 sQueryString);
      printf ("\n\n");
      free (sAction);
      return EXIT_FAILURE;
...
// test for a vaiid action parameter argument in QUERY_STRING ----------------------------------------------------------
  ...
  if ((strcmp (sAction, "Load") != 0) && (strcmp (sAction, "Spin") != 0)
      && (strcmp (sAction, "Pull") != 0) && (strcmp (sAction, "Look") != 0)
      && (strcmp (sAction, "Clear") != 0))
    {
      printf
	("Invalid action: QUERY_STRING=\"%s\" (not \"Action=Load\" or \"Spin\" or \"Pull\" or \"Look\" or \"Clear\"). Ending json4Ajax.cgi",
	 sQueryString);
      printf ("\n\n");
      free (sAction);
      return EXIT_FAILURE;
...
// Initialize a connection and connect to the database -----------------------------------------------------------------
  ...
  if (!mysql_real_connect
      (conn, sgServer, sgUsername, sgPassword, sgDatabase, 0, NULL, 0))
    {
      printf ("\n");
      printf ("Failed to connect to MySQL Server %s in module %s()", sgServer,
	      __func__);
      printf ("\n\n");
      printf ("Error: %s\n", mysql_error (conn));
      printf ("\n");
...
// set a SQL query to insert the new author ----------------------------------------------------------------------------
  ...
  sprintf (caSQL,
	   "SELECT `Gun ID`, `Game No`, `Round No`, `Chamber No`, `Live Round`, `Sound`"
	   "FROM risingfast.`Game Russian Roulette` " "WHERE `Gun ID` = 1");
...
// identify the Action parameter name and value and store the value back in the same local variable 'sAction' ----------
  ...
  if (strcmp (sAction, "Load") == 0)
    {
      ...
      iCompletedRounds = 0;
      if (bLive == true)
	{			// a game is finished if a loaded gun is reloaded
	  iCompletedGames++;
	}
      bLive = true;
      strcpy (caSound, "Loading");
      strcpy (caErrMsg, "No errors");
      strcpy (caResult, "Success");
    }
  else if (strcmp (sAction, "Spin") == 0)
    {				// the state of a bullet is unchanged by spinning
  ...
  else if (strcmp (sAction, "Look") == 0)
    {				// no action is taken with a Look
      strcpy (caSound, "Looking");
      strcpy (caErrMsg, "No Errors");
      strcpy (caResult, "Success");
    }
  else if (strcmp (sAction, "Clear") == 0)
    {				// clear the gun and game statistics
  ...
  else
    {
      ...
      strcpy (caErrMsg, "Unknown action submitted. Nothing done");
...
// set a SQL query to save the new game settings -----------------------------------------------------------------------
  ...
  sprintf (caSQL,
	   "UPDATE risingfast.`Game Russian Roulette` SET `Game No` = %d "
	   ", `Round No` = %d " ", `Chamber No` = %d " ", `Live Round` = %d "
	   ", `Sound` = '%s' " ", `Result` = '%s'", iCompletedGames,
	   iCompletedRounds, iChamber, bLive, caSound, caResult);
...
// create json strings and build and print the JSON response ----------- -----------------------------------------------
  ...
  json_object_object_add (jsonCgiResp, "CompletedGameNo",
			  jsonCompletedGameNo);
  json_object_object_add (jsonCgiResp, "CompletedRoundNo",
			  jsonCompletedRoundNo);
